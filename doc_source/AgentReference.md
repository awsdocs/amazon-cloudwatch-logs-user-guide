...
1. Do one of the following:
   ...
   1. For a new installation of the CloudWatch Logs agent, run the following commands:
      ...
      <a name="ecs_task_iam_role"></a>
      **Note**  
      
      For ECS Tasks (containers) running in Fargate environment, and using Task Role to pass IAM Credentials to the ECS Task/Container, you'll need to tweak the `awslogs-agent-launcher.sh` a little. Usually located in `/var/awslogs/bin/`, the version generated by the `awslogs-agent-setup.py`, will purge (`/usr/bin/env -i`) the environment variables, including the ones carrying the IAM Credential url (AWS_CONTAINER_CREDENTIALS_RELATIVE_URI).
      
      You could use something like (at least at time of this writing, where version of launcher script is: 1.4.3), that would update the launcher script to include the CREDENTIAL env variable.
      ```
      sudo sed -i.bak  's!^/usr/bin/env -i !AWS_CRED=`strings /proc/1/environ | grep AWS_CONTAINER_CREDENTIALS_RELATIVE_URI`\n/usr/bin/env -i ${AWS_CRED}! '  /var/awslogs/bin/awslogs-agent-launcher.sh
      ```
...
**Caught exception: Unable to locate credentials Error when in ECS Fargate environment with ECS Task Role**
If passing AWS Credentials using a IAM Role attached to ECS Task Role, on a ECS Fargate environment, check if the `awslogs-agent-launcher.sh` is not stripping the CREDENTIAL environment variable. See secction [above](#ecs_task_iam_role).
